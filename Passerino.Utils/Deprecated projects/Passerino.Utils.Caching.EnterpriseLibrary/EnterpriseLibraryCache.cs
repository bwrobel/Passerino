...
namespace Passerino.Utils.Caching.EnterpriseLibrary
{
    [Serializable]
    public class EnterpriseLibraryCache : ICache
    {
        ...
        public EnterpriseLibraryCache(IConfigManager configManager, ILogFactory logFactory)
        {
            _defaultCacheMinutes = configManager.GetAppSetting<int>("EnterpriseLibrary_DefaultCacheMinutes", x => x >= -1);
        ...
        /// </summary>
        public T Get<T>(CacheArguments cacheArguments, T defaultValueIfFunctionReturnsNull = default(T))
        {
            ...
            T defaultValueIfFunctionReturnsNull = default(T))
        {
            ...
            {
                _log.Info("Hit cache for key ({0})", cacheArguments.Key).Proceed();
                return value;
            }
            value = func();
            AddToCache(cacheArguments.Key, value, new TimeSpan(0, cacheArguments.CacheMinutes ?? _defaultCacheMinutes, 0));
            _log.Info("Added to cache for key ({0}) for {1} minutes", cacheArguments.Key,cacheArguments.CacheMinutes).Proceed();
            return value;
        }
